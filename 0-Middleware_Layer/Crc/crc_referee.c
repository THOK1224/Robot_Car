...
uint8_t Verify_CRC8_Check_Sum(uint8_t *pchMessage, uint16_t dwLength)
{
    ...
    uint8_t ucExpected = crc_8(pchMessage, dwLength - 1);
...
void Append_CRC8_Check_Sum(uint8_t *pchMessage, uint16_t dwLength)
{
    ...
    pchMessage[dwLength] = crc_8(pchMessage, dwLength);
}
...
{
    if ((pchMessage == 0) || (dwLength <= 2)) return 0;
    uint16_t wExpected = crc_16(pchMessage, dwLength - 2);
...
{
    if (pchMessage == 0) return;
    uint16_t wCRC = crc_16(pchMessage, dwLength - 2);
